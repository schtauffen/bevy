...
impl GlobalTransform {
    ...
    ///
    /// See also the reciprocal [`Self::div_transform`].
    #[inline]
    #[must_use]
    ...
    ///             *transform = initial.div_transform(*parent_transform);
    ...
    /// ```
    ///
    /// Note that `self.div_transform(transform)` is identical to `self / transform`.
    #[inline]
    #[must_use]
    pub fn div_transform(&self, transform: GlobalTransform) -> Transform {
...
impl Div<GlobalTransform> for GlobalTransform {
    type Output = Transform;

    #[inline]
    fn div(self, global_transform: GlobalTransform) -> Self::Output {
        self.div_transform(global_transform)
    }
}

impl Div<Transform> for GlobalTransform {
    type Output = Transform;

    #[inline]
    fn div(self, transform: Transform) -> Self::Output {
        self.div_transform(transform.into())
    }
}
...
mod test {
    use super::*;
    #[test]
    fn div_mul_transform_identity() {
        fn identity(t1: GlobalTransform, t2: GlobalTransform) -> Transform {
            // Equivalent to `t2 * t1 / t2`
            t2.mul_transform(t1.into()).div_transform(t2)
        }
